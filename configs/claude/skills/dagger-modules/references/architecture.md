...
Experimental: annotate service-returning functions with `// +up` and run `dagger up` to start all of a workspace's services on the host at once, instead of the per-function `dagger call <svc> up` form above.
...
Always include the scheme. Passing a bare, scheme-less value is deprecated and warns that a scheme prefix (e.g. `env://`) will be required in a future release.

Secret references (and other function arguments) can be saved as user-defaults in a local `.env` file. Dagger persists the *reference* (e.g. `env://GITHUB_TOKEN`), never the plaintext, so the `.env` holds no secret material.
...
The `// +generate` annotation registers the function with the `dagger generate` command, which runs every generator and merges all of their changesets into the source directory. Each `// +generate` function also gets a matching check automatically, so `dagger check` reports whether generated files are stale (committed output drifted from a fresh run). Use `dagger check --no-generate` to run only the explicit checks, or a toolchain's `ignoreChecks` to suppress its generated checks.

For changesets returned without `// +generate`, the CLI prompts the user to apply them. Use `-y` / `--auto-apply` to apply automatically (useful in CI). Functions can return an array of changesets for multiple independent sets of changes.
...
- `dagger check` (and `dagger generate`) resolve against the entire workspace (all modules and toolchains it contains), not just the current module
- `--failfast` cancels remaining checks on the first failure
- `// +generate` functions contribute their own checks; skip those with `dagger check --no-generate` (see [Changesets](#changesets))
...
| `.WithModel(model)` | Set the model (e.g., `"claude-sonnet-4-6"`, `"gpt-4o"`) |
...
Workspaces support opt-in lockfiles that pin otherwise-floating references (the images resolved by `container.From()` and the commits behind Git refs) so builds are reproducible. Lock state lives in `.dagger/lock`. Control resolution with `--lock`:

```shell
dagger call build                  # disabled: lockfile not consulted (default)
dagger call build --lock live      # resolve fresh, then refresh the lockfile
dagger call build --lock pinned    # use locked digests, lock anything new
dagger call build --lock frozen    # fail if any reference is missing from the lockfile
dagger lock update                 # refresh locked digests to current
```

Use `frozen` in CI to guarantee nothing resolves outside the committed lockfile.
