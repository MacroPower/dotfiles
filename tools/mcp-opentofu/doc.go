// Mcp-opentofu is an MCP server (stdio transport) that exposes seven tools:
// search_registry, get_provider_details, get_module_details, get_resource_docs,
// get_datasource_docs, validate, and init.
//
...
// documentation bodies. The validate and init tools shell out to a local
// tofu binary: validate runs `tofu validate` and renders diagnostics as
// Markdown; init runs `tofu init` to download providers and modules.
//
...
//     (validate, init) (default: "tofu", resolved via PATH when not
//     absolute)
package main
