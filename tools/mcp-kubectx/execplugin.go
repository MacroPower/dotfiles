...
// (`mcp-kubectx exec-plugin --socket <path>`); only the per-`serve`
// socket path varies. Both host- and guest-side serves write the
// same shape, so kubectl never sees the workmux host-exec wrapper.
// The wrapper still happens server side: guest serve's
// [*handler.defaultRunHost] decides between a direct fork and
// `workmux host-exec` for the in-process token call, transparent
// to the kubeconfig.
//
...
func buildExecPlugin(socketPath string) execPlugin {
	return execPlugin{
