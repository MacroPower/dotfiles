...
func TestCheck(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"under ceiling":               {command: "sleep 5"},
		"at ceiling":                  {command: "sleep 10"},
		...
		"foreground loop with short sleep is a documented non-goal": {
...
func TestCheckReason(t *testing.T) {
	...
	t.Run("over ceiling", func(t *testing.T) {
		...
		assert.Contains(t, reason, "10s or less is still allowed")
