...
DIRNAME=$(dirname "$0")
PKG_FILE="$DIRNAME/leanspec-cli.nix"
...
echo "leanspec-cli: current=$currentVersion latest=$latestVersion"
if [[ $currentVersion == "$latestVersion" && $FORCE == false ]]; then
  echo "leanspec-cli is up-to-date"
  exit 0
fi
...
echo "leanspec-cli version + src hash updated to $latestVersion."
