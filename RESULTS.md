...
**Source:** [github.com/srid/nixos-unified](https://github.com/srid/nixos-unified)

A flake-parts module (not a standalone library) that provides a unified API for managing NixOS, nix-darwin, and home-manager configurations within a single flake. Branched from an earlier project called `nixos-flake` in October 2024 (v0.2.0). Backed by Juspay, an Indian fintech company that uses Nix/Haskell extensively.

### Comparison Table

| Aspect                           | srid/nixos-unified                                                                                                                                                                        | Our dotfiles                                                                                                                                                                                                        |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Integration point**            | Flake-parts module you import; provides `mkLinuxSystem`/`mkMacosSystem`/`mkHomeConfiguration` at `self.nixos-unified.lib`                                                                 | `flake-parts.lib.mkFlake` with manual `mkDarwin`/`mkHome`/`mkNixOS` functions defined inline in `flake.nix`                                                                                                         |
| **Builder functions**            | `mkLinuxSystem { home-manager ? false }: mod`, `mkMacosSystem { home-manager ? false }: mod`, `mkHomeConfiguration pkgs: mod`. Each takes a single NixOS/darwin module as the config.     | `mkDarwin { username, homebrew, homeModule }`, `mkHome { system, homeModule }`, `mkNixOS { system, hostModule, username, homeModule }`. Domain-specific params baked into the function signatures.                  |
| **specialArgs**                  | Injects `flake = { self, inputs, config }` into all NixOS/darwin/HM modules automatically. Modules destructure `{ flake, ... }` to access inputs. Also provides `rosettaPkgs` for Darwin. | Not used; inputs accessed via closures in `flake.nix`. Modules do not receive `flake` or `inputs` directly.                                                                                                         |
| **Home-manager wiring**          | Built-in: `{ home-manager = true }` flag auto-configures HM integration with `useGlobalPkgs = true`, `useUserPackages = true`, homeDirectory inference based on username + platform.      | Manual: each `mk*` function explicitly adds `home-manager.*Modules.home-manager`, sets `useGlobalPkgs`, `useUserPackages`, `backupFileExtension`, `sharedModules`. `mkHomeManagerBlock` helper reduces duplication. |
| **Config shape**                 | Each configuration is a single NixOS/darwin module. User config via custom flake-parts options (like a `me` module) accessible everywhere through `flake.config.*` specialArgs.           | `hostConfig` attrset with `username`, `homebrew`, `homeModule`, etc. passed directly to helper functions. Per-host values set in `flake.nix` call sites.                                                            |
| **Shared config**                | Via flake-parts module options accessible through `flake.config.*` specialArgs. Custom option modules (e.g., `me.username`, `me.email`) defined once, available everywhere.               | Via `hostConfig` attrset fields, `sharedOverlays` list, `hmSharedModules` list, `sharedStylixConfig` import. All defined as `let` bindings in `flake.nix`.                                                          |
| **Directory layout**             | Conventional: `configurations/{nixos,darwin,home}/`, `modules/{nixos,darwin,home,flake}/`, `packages/`, `overlays/`                                                                       | Custom: `hosts/`, `home/`, `configs/`, `pkgs/`, `toolchains/`                                                                                                                                                       |
| **Autowiring**                   | Optional `flakeModules.autoWire` scans directories and auto-creates flake outputs from file names. `configurations/nixos/foo.nix` becomes `nixosConfigurations.foo`.                      | None; explicit imports everywhere. Matches our CLAUDE.md preference for explicit configuration.                                                                                                                     |
| **Activation**                   | `nix run .#activate` Nushell script. Supports local activation (auto-detects hostname), remote SSH deployment, home-manager activation (`nix run .#activate $USER@`), and `--dry-run`.    | `task switch` delegates to `nh`. Auto-detects platform. No remote deployment support.                                                                                                                               |
| **Remote deployment**            | Built in via SSH (`nix run .#activate myhost`). Lightweight alternative to deploy-rs/colmena -- no rollback support. Reads `nixos-unified.sshTarget` option from host config.             | Not built in. NixOS hosts (OrbStack, TrueNAS) managed locally.                                                                                                                                                      |
| **Update command**               | `nix run .#update` with configurable `primary-inputs` list (default: nixpkgs, home-manager, nix-darwin).                                                                                  | `task update` runs `nix flake update`.                                                                                                                                                                              |
| **Platform detection**           | `mkLinuxSystem`/`mkMacosSystem` rely on `nixpkgs.hostPlatform` inside the module. `activate` script auto-detects current hostname and matches against available configs.                  | `mkDarwin` hardcodes `aarch64-darwin`; `mkNixOS`/`mkHome` take `system` as a param.                                                                                                                                 |
| **Flake-parts dependency**       | Core dependency; the entire project is a flake-parts module.                                                                                                                              | Yes (used for `perSystem`, `treefmt`, `checks`), but builder functions are plain Nix, not flake-parts modules.                                                                                                      |
| **Stylix integration**           | Not handled; user responsibility.                                                                                                                                                         | Deep integration: `stylix.darwinModules.stylix`, `stylix.nixosModules.stylix`, `stylix.homeModules.stylix` imported in each helper with shared `hosts/stylix.nix` config.                                           |
| **Homebrew integration**         | Not handled; user responsibility.                                                                                                                                                         | Deep integration: `nix-homebrew.darwinModules.nix-homebrew` with `enableRosetta`, `autoMigrate`, per-host `homebrew` attrset.                                                                                       |
| **Secrets integration**          | Not handled; user responsibility.                                                                                                                                                         | `sops-nix.homeManagerModules.sops` in shared HM modules list.                                                                                                                                                       |
| **Domain-specific modules**      | None. Generic builder functions only.                                                                                                                                                     | `krewfile`, `nix-index-database`, `llm-agents`, `dagger` overlays -- all wired into helpers.                                                                                                                        |
| **Boilerplate**                  | Low -- `mkLinuxSystem`/`mkMacosSystem` are one-liners; autowiring eliminates manual output wiring. With `lib.mkFlake`, entire `flake.nix` can be ~10 lines.                               | Medium -- each `mk*` helper is ~30-50 lines, but defined once. Per-host configs are concise call sites.                                                                                                             |
| **Transparency**                 | Medium -- some magic in specialArgs injection and autowiring; need to understand the module internals. The `flake` specialArg is not obvious without reading nixos-unified source.        | High -- all wiring is visible in `flake.nix`. No hidden specialArgs or auto-discovery.                                                                                                                              |
| **Additional dependencies**      | None beyond flake-parts (nushell built from nixpkgs for activate script).                                                                                                                 | None beyond flake-parts.                                                                                                                                                                                            |
| **homeConfigurations placement** | Under `legacyPackages.${system}.homeConfigurations` (to work around HM requiring `pkgs`). Slightly awkward.                                                                               | Top-level `homeConfigurations` (standard placement).                                                                                                                                                                |
| **Per-host metadata**            | `nixos-unified.sshTarget` and `nixos-unified.overrideInputs` options on each host config. Read-only computed `outputs.*` for deploy tooling.                                              | No per-host metadata options. Deployment info not tracked in Nix config.                                                                                                                                            |
| **HM defaults baked in**         | `homeDirectory` auto-set from username + platform. `sessionPath` includes Nix profile paths on macOS.                                                                                     | `homeDirectory` set manually per platform in `mkDarwin`/`mkNixOS` inline modules.                                                                                                                                   |
| **Project maturity**             | Active since 2023 (as nixos-flake), renamed 2024. Small user base (primarily Juspay/srid ecosystem). 70+ stars.                                                                           | N/A (our repo).                                                                                                                                                                                                     |

### Migration Effort Assessment

Adopting nixos-unified would require:

1. **Adding flake input:** `nixos-unified.url = "github:srid/nixos-unified";`
2. **Importing flake module:** Add `nixos-unified.flakeModules.default` to flake-parts imports.
3. **Rewriting helpers:** Replace `mkDarwin`/`mkNixOS`/`mkHome` with `self.nixos-unified.lib.mkMacosSystem`/`mkLinuxSystem`/`mkHomeConfiguration`. Each host config becomes a single NixOS/darwin module rather than a `hostConfig` attrset.
4. **Moving domain integrations:** Stylix, nix-homebrew, sops-nix, krewfile, nix-index-database imports would move from the helper functions into individual host configuration modules or shared modules imported by each host.
5. **Adapting to specialArgs:** Modules would receive `{ flake, ... }` instead of relying on closures. The `dotfiles.*` options system would continue to work unchanged since it is a NixOS/HM module system feature, not a flake-level concern.

The migration is non-trivial because our helpers embed domain-specific integrations (Stylix, nix-homebrew, sops-nix, krewfile) that nixos-unified does not handle. These integrations would need to be factored out into shared modules that each host imports, which is more verbose than our current approach of baking them into the helper functions.

### Candidate Changes

1. **Adopt specialArgs pattern for `flake` access in modules**
   - **Rationale:** nixos-unified injects `flake = { self, inputs, config }` as specialArgs so any NixOS/darwin/HM module can destructure `{ flake, ... }` to access flake inputs and config. This makes modules self-contained and portable -- they do not depend on closure scope from `flake.nix`. We could add `specialArgs = { flake = { inherit self inputs; }; }` to our `mkDarwin`/`mkNixOS` helpers and `extraSpecialArgs` to `mkHome` without any external dependency. This is the most valuable pattern from nixos-unified and is trivially adoptable.
   - **Source:** `nix/modules/flake-parts/lib.nix` (specialArgsFor definition)
   - **Impact:** Medium. Improves module portability and makes input access explicit rather than closure-dependent. Two-repo signal for this pattern (srid, kclejeune's `primaryUser.nix` also passes data via specialArgs).

2. **Auto-infer `homeDirectory` from username and platform**
   - **Rationale:** nixos-unified computes `home.homeDirectory` automatically: `/Users/${username}` on Darwin, `/home/${username}` on Linux. We currently set this manually in platform-specific inline modules within `mkDarwin` and `mkNixOS`. The inference is trivial and eliminates a source of copy-paste errors. Could be added to `mkHomeManagerBlock` or each helper function.
   - **Source:** `nix/modules/flake-parts/lib.nix` (homeModules.common)
   - **Impact:** Low. Small quality-of-life improvement; we already handle this correctly but redundantly.

3. **Create a shared flake-parts config module for user identity**
   - **Rationale:** srid's personal config defines a `me` flake-parts option module (`me.username`, `me.fullname`, `me.email`, `me.sshKey`) that is accessible everywhere via `flake.config.me`. This is cleaner than our current approach where `dotfiles.git.userName`/`userEmail` must be set per-host in `flake.nix`. A flake-parts config module would define user identity once and make it available to all NixOS/darwin/HM modules via specialArgs. However, this only works for single-user configs; our setup supports different usernames per host (jacobcolvin vs. jcolvin) and different emails, so a single `me` module would not directly fit.
   - **Source:** srid/nixos-config `nix/modules/flake-parts/config.nix`
   - **Impact:** Low. Our per-host differentiation (different usernames, different emails) means a single `me` module is insufficient. The `dotfiles.*` options approach is more flexible for our use case.

4. **Evaluate `nix run .#activate` pattern for unified activation**
   - **Rationale:** The activate script wraps `darwin-rebuild switch`, `nixos-rebuild switch`, and `home-manager switch` behind a single `nix run .#activate` command that auto-detects the current hostname. It also supports remote deployment via SSH. Our `task switch` already handles local auto-detection via `nh`, but the remote deployment feature could be useful for our TrueNAS host. The implementation is a Nushell script packaged as a flake app.
   - **Source:** `activate/activate.nu`, `nix/modules/flake-parts/packages.nix`
   - **Impact:** Low. Our `task switch` via `nh` already provides a good local activation UX. Remote deployment via SSH could be added to our Taskfile without adopting the full nixos-unified activate script. deploy-rs or colmena are more robust for remote deployment (they support rollback).

5. **Add `nix.settings.max-jobs = "auto"` to shared Nix config**
   - **Rationale:** nixos-unified sets `nix.settings.max-jobs = "auto"` as a default, which tells the Nix daemon to use all available CPU cores for builds. This is a simple optimization we do not currently set in `hosts/shared.nix`. The default without this setting is 1 on some platforms.
   - **Source:** `nix/modules/flake-parts/lib.nix` (nixosModules.nix)
   - **Impact:** Low. One-line addition to `hosts/shared.nix`. Improves build parallelism on multi-core machines.

6. **Consider flake-parts option modules for cross-cutting config**
   - **Rationale:** nixos-unified demonstrates that flake-parts option modules (defined as NixOS-style modules with `options` and `config`) can serve as a configuration layer above NixOS/darwin/HM. The `nixos-unified.sshTarget` option is an example: it attaches deploy metadata to host configs without polluting the NixOS option namespace. We could use this pattern for our own cross-cutting concerns (e.g., a `dotfiles.deploy.sshTarget` option on NixOS hosts, or a `dotfiles.role` enum for host classification). However, this adds abstraction complexity and is only justified if we have multiple cross-cutting concerns to manage.
   - **Source:** `nix/modules/configurations/default.nix`
   - **Impact:** Low. Not immediately needed given our small host count. Worth revisiting if host count or deployment complexity grows.

7. **Skip full nixos-unified adoption**
