...
// TestSandboxBase verifies that [Dev.SandboxBase] layers the terrarium
// config, environment, and user setup on top of the base container.
// Checks that config YAML exists, terrarium binary is on PATH, required
// binaries are reachable, the non-root user exists, and IS_SANDBOX_NETWORK
// is set.
//
// +check
func (m *Tests) TestSandboxBase(ctx context.Context) error {
	...
	// Verify config YAML exists with egress rules.
	cfgContent, err := ctr.File(terrariumConfigPath).Contents(ctx)
	if err != nil {
		return fmt.Errorf("reading config.yaml: %w", err)
	}
	for _, want := range []string{"egress", "toFQDNs", "matchName", "anthropic.com", "github.com", "golang.org"} {
		if !strings.Contains(cfgContent, want) {
			return fmt.Errorf("config.yaml missing %q", want)
		}
	}

	// Verify terrarium binary is on PATH and executable.
	if _, err := ctr.WithExec([]string{"which", "terrarium"}).Sync(ctx); err != nil {
		return fmt.Errorf("terrarium not on PATH: %w", err)
	}
	...
	// Verify that binaries referenced by terrarium init are reachable.
	...
	// Verify non-root user exists in /etc/passwd.
	passwd, err := ctr.File("/etc/passwd").Contents(ctx)
	if err != nil {
		return fmt.Errorf("reading /etc/passwd: %w", err)
	}
	if !strings.Contains(passwd, "dev:x:1000:1000") {
		return fmt.Errorf("/etc/passwd missing dev user entry")
	}
...
// writes only the provided config YAML (not the default domains).
//
// +check
...
`),
	...
	// Check config.yaml contains the specified domains.
	cfgContent, err := ctr.File(terrariumConfigPath).Contents(ctx)
	if err != nil {
		return fmt.Errorf("reading config.yaml: %w", err)
	}
	...
	// Verify terrarium can generate config from the provided YAML.
	ctr = ctr.WithExec([]string{"terrarium", "generate"})
	...
	for _, unwanted := range []string{"github.com", "anthropic.com"} {
		if strings.Contains(envoyConf, unwanted) {
			return fmt.Errorf("generated envoy config should not contain %s with only go domains", unwanted)
		}
	}
...
// TestSandboxAdditionalPorts verifies that extra ports in the config YAML
// produce the expected terrarium generate output with iptables REDIRECT
// rules and an Envoy TLS passthrough listener.
//
// +check
...
// TestSandboxTCPForward verifies that tcpForwards in the config YAML
// produces the expected terrarium generate output with TCP forward
// listener, STRICT_DNS cluster, and iptables redirect rule.
//
// +check
...
// TestSandboxLogging verifies that enabling sandbox logging in the config
// YAML produces terrarium generate output with access logs in Envoy and
// LOG targets in iptables drop rules.
//
// +check
...
// in the config YAML produce terrarium generate output with Envoy config
// containing :method header matchers and 403 catch-all routes. Also
// verifies that MITM certs are generated for method-only restricted domains.
//
// +check
...
// by activating nftables, dnsmasq, and Envoy via terrarium init and
// testing real traffic. Unlike [Tests.TestSandboxBase] (which only checks
// config presence), this test loads the firewall and validates that
// allowed domains are reachable, blocked domains fail to resolve, and
// direct IP connections are dropped.
//
// Security review: InsecureRootCapabilities is required because
// terrarium init needs CAP_NET_ADMIN to load nftables rules and start
...
`
	...
	_, err := ctr.
		...
		WithExec(
			[]string{"terrarium", "init", "--", "sh", "-c", script},
...
//
// Security review: InsecureRootCapabilities is required because
// terrarium init needs CAP_NET_ADMIN. The init script drops to uid 1000
...
// container with terrarium binary, default config.yaml, correct
...
func (m *Tests) TestPublishSandboxImage(ctx context.Context) error {
	...
	if _, err := ctr.WithExec([]string{"which", "terrarium"}).Sync(ctx); err != nil {
		return fmt.Errorf("terrarium not found: %w", err)
	}
...
// TestSandboxGenDefault verifies that terrarium generate with the default
// config.yaml produces the expected output files.
//
// Not annotated with +check because it requires building the full image.
// Run manually:
//
...
// TestSandboxGenCustomConfig verifies that terrarium generate with a custom
...
`
	...
	ctr := dag.Container().From(addr).
		WithNewFile(terrariumConfigPath, customConfig).
		WithExec([]string{"terrarium", "generate"})
...
`),
	...
	// Run terrarium generate to produce the dnsmasq config.
	ctr = ctr.WithExec([]string{"terrarium", "generate"})
